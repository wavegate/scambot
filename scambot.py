...
class App(tk.Tk):
    ...
    def create_widgets(self):
        ...
        self.create_button_export_finds()
    ...
    def create_search_results(self):
        ...
        self.results_frame.grid(row=0, column=0, columnspan=14, rowspan=5, padx=5, pady=5, sticky='nsew')

        self.results_scroll = tk.Scrollbar(self.results_frame)
        self.results_scroll.pack(side='right', fill='y')

        self.results_text = tk.Text(self.results_frame, padx=5, pady=5)
    ...
    def create_finds(self):
        self.finds_frame = tk.Frame(self, bd=1, relief='sunken')
        self.finds_frame.grid(row=0, column=14, columnspan=6, rowspan=10, padx=5, pady=5, sticky='nsew')

        self.finds_scroll = tk.Scrollbar(self.finds_frame)
        self.finds_scroll.pack(side='right', fill='y')

        #self.finds_scroll.configure(command=self.finds_text.yview)
        #self.finds_text['yscrollcommand'] = self.finds_scroll.set

    def create_searches(self):
        self.searches_frame = tk.Frame(self, bd=1, relief='sunken')
        self.searches_frame.grid(row=5, column=0, columnspan=14, rowspan=5, padx=5, pady=5, sticky='nsew')

        self.searches_scroll = tk.Scrollbar(self.searches_frame)
        self.searches_scroll.pack(side='right', fill='y')

        self.searches_text = tk.Text(self.searches_frame, padx=5, pady=5)
        self.searches_text.pack(expand=True, fill='both')
        self.searches_text.configure(state='disabled')

        self.searches_scroll.configure(command=self.searches_text.yview)
        self.searches_text['yscrollcommand'] = self.searches_scroll.set

    def create_params_notebook(self):
        #print("create_params_notebook")
        """Creates the notebook (tabs) for each set of search parameters"""
        self.params_notebook = ttk.Notebook(self)
        self.params_notebook.grid(row=0, column=14, columnspan=6, rowspan=10, padx=5, pady=5, sticky='nsew')
    ...
    def create_button_add(self):
        self.button_add = ttk.Button(self, text='Add', command=self.add_search, width=0)
        self.button_add.grid(row=constants.RESULTS_HEIGHT, column=constants.RESULTS_WIDTH + 2, padx=5, pady=5, sticky='ew')

    def create_button_export_finds(self):
        self.button_add = ttk.Button(self, text='Export Finds', command=self.export_finds, width=0)
        self.button_add.grid(row=constants.RESULTS_HEIGHT, column=constants.RESULTS_WIDTH + 3, columnspan=2, padx=5, pady=5, sticky='ew')

    def add_search(self):
        #self.searches.append(self.search_pages[0].get_params())
    ...
    def update_searches(self):
        self.handle_print_searches(str(self.searches))
    ...
    def handle_print_searches(self, string):
        #print("handle_print")
        """Handles printing to the results pane."""
        new_string = time.strftime('[%y-%m-%d %H:%M:%S] ') + string + '\n'
        scroll = (self.searches_scroll.get()[1] == 1.0)

        self.searches_text.configure(state='normal')
        self.searches_text.insert('end', new_string)
        while float(self.searches_text.index('end')) > self.max_console_size + 2:
            self.searches_text.delete(1.0, 2.0)
        self.searches_text.configure(state='disabled')

        if scroll:
            self.searches_text.see('end')
    ...
    def export_finds(self):
        self.filename = open("finds.txt", 'w')
        self.filename.write("\n".join(str(self.finds)))
        self.filename.close()
        self.handle_print_searches(str(self.finds))

    def make_nice_price(self, to_parse):
        #print("make_nice_price")
